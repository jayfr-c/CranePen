...
{ 
	...
	public void SetGrid(GridArea<HeatMapGridObject> gridArea){
	...
	private void Grid_OnGridValueChanged(object sender, GridArea<HeatMapGridObject>.OnGridValueChangedEventArgs e){
	...
	private void UpdateHeatMapVisual(){
		...
		for(int x = 0; x < gridArea.GetWidth(); x++){
			for(int y = 0; y < gridArea.GetHeight(); y++){
				...
				HeatMapGridObject gridObject = gridArea.GetGridObject(x, y);
